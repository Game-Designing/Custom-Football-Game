package com.paypal.android.sdk;

import android.content.Context;
import android.util.TypedValue;
import android.view.View;
import android.view.ViewGroup;
import android.view.ViewGroup.LayoutParams;
import android.widget.Button;
import android.widget.ImageView;
import android.widget.ImageView.ScaleType;
import android.widget.LinearLayout;
import android.widget.TextView;

/* renamed from: com.paypal.android.sdk.ac */
public final class C11791ac {

    /* renamed from: a */
    public ViewGroup f37120a;

    /* renamed from: b */
    public Button f37121b;

    /* renamed from: c */
    public C11821fc f37122c;

    /* renamed from: d */
    public TextView f37123d;

    /* renamed from: e */
    private LinearLayout f37124e = C11715Da.m38772a(this.f37120a);

    /* renamed from: f */
    public TextView f37125f;

    /* renamed from: g */
    public TextView f37126g;

    /* renamed from: h */
    public ImageView f37127h;

    /* renamed from: i */
    public LinearLayout f37128i;

    /* renamed from: j */
    public C11767Tb f37129j;

    /* renamed from: k */
    public LinearLayout f37130k;

    /* renamed from: l */
    public Button f37131l;

    /* renamed from: m */
    public TextView f37132m;

    /* renamed from: n */
    public ImageView f37133n;

    public C11791ac(Context context) {
        this.f37120a = C11715Da.m38769a(context);
        LinearLayout b = C11715Da.m38787b((ViewGroup) this.f37124e);
        this.f37132m = new TextView(context);
        this.f37132m.setText("Your Order");
        String str = "0dip";
        C11715Da.m38781a(this.f37132m, str, str, str, "14dip");
        this.f37132m.setTextSize(24.0f);
        this.f37132m.setTextColor(C11711Ca.f36616e);
        b.addView(this.f37132m, new LayoutParams(-2, -2));
        this.f37122c = new C11821fc(context, "$0.00");
        b.addView(this.f37122c.f37206b);
        String str2 = "16dip";
        C11715Da.m38780a((View) this.f37122c.f37206b, (String) null, str2);
        this.f37130k = C11715Da.m38771a(context, true, 42044, b);
        ImageView imageView = new ImageView(context);
        imageView.setImageBitmap(C11715Da.m38792c("", context));
        imageView.setScaleType(ScaleType.CENTER_INSIDE);
        imageView.setAdjustViewBounds(true);
        String str3 = "PayPal";
        imageView.setContentDescription(str3);
        String str4 = "28dip";
        String str5 = "2.5dip";
        String str6 = "4dip";
        if (C11756Pb.f36876a) {
            this.f37130k.addView(imageView);
            C11715Da.m38789b(imageView, str6, str5, null, null);
            C11715Da.m38778a((View) imageView, -2, str4);
        }
        this.f37126g = new TextView(context);
        C11715Da.m38784a(this.f37126g);
        this.f37126g.setText("Pay with");
        this.f37130k.addView(this.f37126g);
        if (!C11756Pb.f36876a) {
            this.f37130k.addView(imageView);
            C11715Da.m38789b(imageView, str6, str5, null, null);
            C11715Da.m38778a((View) imageView, -2, str4);
        }
        LinearLayout linearLayout = new LinearLayout(context);
        linearLayout.setOrientation(1);
        b.addView(linearLayout, new LinearLayout.LayoutParams(-1, -2));
        C11715Da.m38789b(linearLayout, null, null, null, str2);
        this.f37123d = new TextView(context);
        this.f37123d.setTextSize(14.0f);
        this.f37123d.setTextColor(C11711Ca.f36636y);
        this.f37123d.setGravity(17);
        linearLayout.addView(this.f37123d, new LinearLayout.LayoutParams(-1, -2));
        C11715Da.m38789b(this.f37123d, null, "8dip", null, str6);
        this.f37121b = new Button(context);
        linearLayout.addView(this.f37121b);
        C11715Da.m38783a(this.f37121b);
        C11715Da.m38778a((View) this.f37121b, -2, "24dip");
        C11715Da.m38776a((View) this.f37121b, 17, 1.0f);
        this.f37128i = C11715Da.m38771a(context, false, 42043, b);
        this.f37133n = new ImageView(context);
        this.f37133n.setAdjustViewBounds(true);
        this.f37133n.setContentDescription(str3);
        this.f37133n.setScaleType(ScaleType.CENTER_INSIDE);
        this.f37133n.setImageBitmap(C11715Da.m38792c("", context));
        this.f37128i.addView(this.f37133n, new LayoutParams(-2, (int) TypedValue.applyDimension(1, 22.0f, context.getResources().getDisplayMetrics())));
        C11715Da.m38789b(this.f37133n, null, null, "6dip", "1dip");
        C11715Da.m38778a((View) this.f37133n, -2, "22dip");
        this.f37133n.setVisibility(8);
        this.f37127h = new ImageView(context);
        this.f37127h.setAdjustViewBounds(true);
        this.f37127h.setContentDescription(str3);
        this.f37127h.setScaleType(ScaleType.CENTER_INSIDE);
        this.f37127h.setVisibility(8);
        this.f37128i.addView(this.f37127h, new LayoutParams(-2, (int) TypedValue.applyDimension(1, 22.0f, context.getResources().getDisplayMetrics())));
        C11715Da.m38789b(this.f37127h, null, null, str6, null);
        C11715Da.m38788b(this.f37127h, "36dip", "25dip");
        this.f37125f = new TextView(context);
        C11715Da.m38784a(this.f37125f);
        this.f37125f.setText("Pay with Card");
        this.f37128i.addView(this.f37125f, new LayoutParams(-2, -2));
        this.f37131l = new Button(context);
        this.f37131l.setId(42042);
        C11715Da.m38783a(this.f37131l);
        b.addView(this.f37131l);
        C11715Da.m38789b(this.f37131l, null, null, null, str2);
        C11715Da.m38777a((View) this.f37131l, -2, -2);
        C11715Da.m38776a((View) this.f37131l, 17, 1.0f);
        this.f37129j = new C11767Tb(context);
        b.addView(this.f37129j.f37034a);
        C11715Da.m38777a((View) this.f37129j.f37034a, -2, -2);
        C11715Da.m38776a((View) this.f37129j.f37034a, 17, 1.0f);
    }
}
